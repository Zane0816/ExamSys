...
class Home extends React.Component<HomeProps> {
  ...
  componentDidMount(): void {
    this.CheckedExamChange = reaction(
      () => AppStore.CheckedExam,
      (CheckedExam) => {
  ...
  render() {
    ...
    if (AppStore.ExamStatus === 1 && AppStore.ExamInfo) {
      return (
        <Form id="ExamList" labelCol={{ span: 3 }} wrapperCol={{ span: 20 }}>
          <header>
            <h1>{AppStore.ExamInfo.Name}</h1>
            <label>总分:{AppStore.ExamInfo.TotalScore}</label>
            {AppStore.ExamCheckResult && <label style={{ marginLeft: 20, color: '#52c41a' }}>得分：{this.Score}</label>}
          </header>
          <Row>
            <Col span={12}>
          ...
          {AppStore.ExamInfo.Questions.map((d, i) => (
            <Form.Item label={`题目${i + 1}`} key={i}>
              <p>
                <label>{d.Title}</label>
              </p>
              ...
              {'Answers' in d
                ? getFieldDecorator(`Question${i}`)(
                    <Checkbox.Group disabled={AppStore.CheckedExam}>
                      {d.Answers.map((da, ai) => (
                        <Checkbox value={Answer[ai]} key={ai}>
                          {Answer[ai]}：{da.Title}
                        </Checkbox>
                      ))}
                    </Checkbox.Group>,
                  )
                : getFieldDecorator(`Question${i}`)(<Input.TextArea rows={4} />)}
            </Form.Item>
          ))}
